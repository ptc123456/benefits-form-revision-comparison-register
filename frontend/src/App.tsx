...
export function App() {
  ...
  return <main className="shell">
    <header className="masthead"><div className="masthead-copy"><p className="eyebrow">FORM / LINE · STUDIONET REGISTER</p><h1>Catch a requirement change before it catches an applicant.</h1><p className="lede">Freeze two structured form revisions. GenLayer validators independently retrieve the sources, normalize the required sets, and record the comparison that survives the review.</p></div><div className="masthead-actions"><div className="network-chip"><span className="pulse" /> Studionet <small>chain 61999</small></div><section className="wallet wallet-header" aria-label="Wallet connection"><div className="wallet-heading"><p className="eyebrow">CONTROL ROOM</p><span className="wallet-status"><i />{connectedLabel}</span></div><p className="muted">Choose a signer to interact with the register.</p><select aria-label="Wallet provider" value={wallet?.info.uuid || ""} onChange={(e) => setWallet(wallets.find((item) => item.info.uuid === e.target.value) || null)}><option value="">Select MetaMask, OKX, or Rabby</option>{wallets.map((item) => <option key={item.info.uuid} value={item.info.uuid}>{item.info.name}</option>)}</select><button className="secondary wide" onClick={connect}>{connectedLabel}</button><div className="rule-list"><span><i />Wallet boundary</span><span><i />Finality checked</span><span><i />State readback checked</span></div></section></div></header>
